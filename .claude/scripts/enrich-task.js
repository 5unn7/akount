...
function loadEnrichments() {
  if (!fs.existsSync(ENRICHMENTS_FILE)) {
    return {
      $comment: 'Manual enrichments for high-risk tasks. Auto-generated enrichments marked with autoGenerated: true',
    };
  }
...
function saveEnrichments(enrichments) {
  fs.writeFileSync(ENRICHMENTS_FILE, JSON.stringify(enrichments, null, 2) + '\n');
}
...
function main() {
  ...
  if (!dryRun && enrichedCount > 0) {
    ...
    console.log(`📄 Updated: ${ENRICHMENTS_FILE}`);
    console.log('\n💡 Next steps:');
