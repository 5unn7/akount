...
const { loadEnrichments: loadEnrichmentsHelper } = require('../../lib/load-enrichments');
...
const CLAIMS_FILE = path.join(PROJECT_ROOT, '.claude/state/task-claims.json');
const BLAME_CACHE_FILE = path.join(PROJECT_ROOT, '.claude/state/.task-blame-cache.json');
const REVIEWS_DIR = path.join(PROJECT_ROOT, 'docs/reviews');
const INBOX_FILE = path.join(PROJECT_ROOT, '.claude/state/task-inbox.json');
const OUTBOX_FILE = path.join(PROJECT_ROOT, '.claude/state/task-outbox.json');
...
function loadEnrichments() {
  // Use helper to load from domain-based structure
  return loadEnrichmentsHelper() || {};
}
