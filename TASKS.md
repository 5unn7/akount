...
| ARCH-18 | Feedback Loop 1: Request timing middleware — Fastify `onRequest`/`onResponse` hooks to measure latency, log slow requests (>500ms) to `.claude/runtime/request-log.json`. Dev-only, zero prod overhead. Register after auth in `apps/api/src/index.ts`. | 1-2h | 🟠 High | 🟢 | | plan:feedback-loops |
| PERF-28 | Feedback Loop 1: Prisma query observer — Hook `prisma.$on('query')` to detect slow queries (>100ms) and N+1 patterns (same fingerprint >5x/request). Write to `.claude/runtime/query-log.json`. Opt-in via `PRISMA_QUERY_LOG=true` env var. | 2-4h | 🟠 High | 🟢 | | plan:feedback-loops |
...
| ID | Task | Effort | Priority | Status | Deps | Source |
|----|------|--------|----------|--------|------|--------|
| ARCH-19 | Feedback Loop 1: Runtime error collector — Wrap existing `errorHandler.ts` to ALSO append unhandled/500 errors to `.claude/runtime/error-log.json` with stack trace (10 lines), request context, dedup (same message within 5min = increment count). Dev-only, keep last 50 entries. | 1h | 🟡 Medium | 🟢 | | plan:feedback-loops |
| ARCH-20 | Feedback Loop 1: Runtime summary script — Reads `.claude/runtime/request-log.json`, `query-log.json`, `error-log.json` and produces human-readable summary for `/processes:begin`. Shows slow endpoints, N+1 queries, unhandled errors since last session. | 1h | 🟡 Medium | 🟢 | [needs: ARCH-18, PERF-28, ARCH-19] | plan:feedback-loops |
