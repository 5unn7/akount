...
| ID | Task | Effort | Priority | Status | Deps | Source |
|----|------|--------|----------|--------|------|--------|
| INFRA-64 | Feedback Loop 4: Production signal schema — Define `.claude/production/signals.json` schema for error/performance/security/deprecation signals from Sentry/Vercel/manual sources. Include: id, timestamp, type, severity, source, message, file, line, frequency, resolved flag. Create README.md documenting how to populate. | 1h | 🟡 Medium | 🟢 | | plan:feedback-loops |
| INFRA-65 | Feedback Loop 4: Production signal reader for `/processes:begin` — Script `read-production-signals.js` that reads `.claude/production/signals.json`, formats unresolved signals with severity/frequency for session start display. Add section to `begin.md` after git status. | 1h | 🟡 Medium | 🟢 | [needs: INFRA-64] | plan:feedback-loops |
| INFRA-66 | Feedback Loop 4: Manual signal ingestion CLI — Script `add-production-signal.js` accepting `--type`, `--severity`, `--message`, `--file`, `--line` flags. Also `--resolve sig-001` to mark resolved. For teams without Sentry integration. | 30m | ⚪ Low | 🟢 | [needs: INFRA-64] | plan:feedback-loops |
