...
| ARCH-27 | Index V2: Split WEB-COMPONENTS (180 files, 16.9K tokens) into 3 sub-indexes: WEB-BUSINESS (~60 files, `components/business/`), WEB-SHARED (~70 files, `components/ui/`, `components/shared/`), WEB-FORMS (~50 files). Update DOMAIN_INDEX_MAP, adjacency matrix, KEYWORD_MAP. | 2h | 🟠 High | 🟢 | | plan:feedback-loops |
| PERF-29 | Index V2: Add caller graph — Add `callers` field per export in code index. Build reverse import map: export → list of importing files. Eliminates "trace impact" Greps (2-3 tool calls per lookup). ~30% token increase per domain. | 2-4h | 🟠 High | 🟢 | | plan:feedback-loops |
...
| INFRA-68 | Index V2: Add test coverage map — Cross-reference service/route files with test files (`X.test.ts` or `__tests__/X.test.ts`). Add `tests` field to index entries: `{ file, exists, testCount }`. ~10% token increase, signals coverage gaps. | 1h | 🟡 Medium | 🟢 | | plan:feedback-loops |
