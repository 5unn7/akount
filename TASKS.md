...
| ARCH-21 | Feedback Loop 2: Git diff pattern extractor — Script that analyzes `git diff` from session to auto-detect: new utilities, cross-domain changes, bug fix patterns, schema changes, new test files. Output to `.claude/session-patterns.json`. Consumed by end-session. | 2-4h | 🟠 High | 🟢 | | plan:feedback-loops |
...
| ARCH-22 | Feedback Loop 2: Enhance end-session with auto-capture — Add Step 1c to `end-session.md` that runs `extract-session-patterns.js`, auto-populates "Patterns Discovered" and "Bugs Fixed" sections. Agent reviews auto-fill (can edit/remove false positives). | 1h | 🟡 Medium | 🟢 | [needs: ARCH-21] | plan:feedback-loops |
| ARCH-23 | Feedback Loop 2: Auto-route patterns to MEMORY topic files — Script `route-to-memory.js` that takes session patterns and appends to correct topic file: bugs → `debugging-log.md`, API patterns → `api-patterns.md`, frontend gotchas → `codebase-quirks.md`. Append-only, dedup by fuzzy match. | 1-2h | 🟡 Medium | 🟢 | [needs: ARCH-21] | plan:feedback-loops |
