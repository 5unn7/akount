...
export class PostingService {
   ...
   */
  async postTransaction(transactionId: string, glAccountId: string) {
    return await prisma.$transaction(async (tx) => {
      ...
      // 8. Source document (allowlisted fields)
      ...
      // 9. Create journal entry (POSTED immediately — auto-approved)
   ...
   */
  async postBulkTransactions(transactionIds: string[], glAccountId: string) {
    return await prisma.$transaction(async (tx) => {
