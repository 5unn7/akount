...
import { normalizeInstitutionName } from './parser.service';
...
export async function matchAccountToBankConnection(
  ...
  for (const account of candidates) {
    ...
    if (
      normalizedAccountName.includes(normalizedBankInstitution) ||
    ...
    if (
      (externalAccountData.type === 'checking' &&
        (accountNameLower.includes('checking') || accountNameLower.includes('chequing'))) ||
      (externalAccountData.type === 'savings' && accountNameLower.includes('savings')) ||
      (externalAccountData.type === 'credit' &&
        (accountNameLower.includes('credit') ||
          accountNameLower.includes('visa') ||
          accountNameLower.includes('mastercard')))
    ) {
      score += 15;
