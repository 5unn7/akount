 ...
 * All routes require authentication and create resources for the authenticated user.
 */
...
export async function onboardingRoutes(fastify: FastifyInstance) {
      ...
      try {
        ...
        const result = await prisma.$transaction(async (tx) => {
          ...
          const defaultAccounts = [
            ...
            { code: '1100', name: 'Accounts Receivable', type: 'ASSET' as const, balance: 'DEBIT' as const },
            { code: '2000', name: 'Accounts Payable', type: 'LIABILITY' as const, balance: 'CREDIT' as const },
            { code: '3000', name: 'Owner Equity', type: 'EQUITY' as const, balance: 'CREDIT' as const },
   ...
   */
  fastify.get<{ Reply: StatusResponse | ErrorResponse }>(
    '/onboarding/status',
    { onRequest: [authMiddleware] },
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
