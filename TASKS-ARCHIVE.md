...
| ARCH-27 | Index V2: Split WEB-COMPONENTS (180 files, 16.9K tokens) into 3 sub-indexes: WEB-BUSINESS (~60 files, `components/business/`), WEB-SHARED (~70 files, `components/ui/`, `components/shared/`), WEB-FORMS (~50 files). Update DOMAIN_INDEX_MAP, adjacency matrix, KEYWORD_MAP. | 2026-02-28 | plan:feedback-loops |
| PERF-29 | Index V2: Add caller graph — Add `callers` field per export in code index. Build reverse import map: export → list of importing files. Eliminates "trace impact" Greps (2-3 tool calls per lookup). ~30% token increase per domain. | 2026-02-28 | plan:feedback-loops |
| ARCH-19 | Feedback Loop 1: Runtime error collector — Wrap existing `errorHandler.ts` to ALSO append unhandled/500 errors to `.claude/runtime/error-log.json` with stack trace (10 lines), request context, dedup (same message within 5min = increment count). Dev-only, keep last 50 entries. | 2026-02-28 | plan:feedback-loops |
| ARCH-20 | Feedback Loop 1: Runtime summary script — Reads `.claude/runtime/request-log.json`, `query-log.json`, `error-log.json` and produces human-readable summary for `/processes:begin`. Shows slow endpoints, N+1 queries, unhandled errors since last session. | 2026-02-28 | plan:feedback-loops |
| ARCH-22 | Feedback Loop 2: Enhance end-session with auto-capture — Add Step 1c to `end-session.md` that runs `extract-session-patterns.js`, auto-populates "Patterns Discovered" and "Bugs Fixed" sections. Agent reviews auto-fill (can edit/remove false positives). | 2026-02-28 | plan:feedback-loops |
| ARCH-23 | Feedback Loop 2: Auto-route patterns to MEMORY topic files — Script `route-to-memory.js` that takes session patterns and appends to correct topic file: bugs → `debugging-log.md`, API patterns → `api-patterns.md`, frontend gotchas → `codebase-quirks.md`. Append-only, dedup by fuzzy match. | 2026-02-28 | plan:feedback-loops |
| ARCH-24 | Feedback Loop 3: Decision journal format & storage — Create `.claude/decisions/` directory with template format (Context, Decision, Alternatives, Consequences, Files Affected, Tags). Auto-generated INDEX.md. Format: `YYYY-MM-DD-short-description.md`. | 2026-02-28 | plan:feedback-loops |
| ARCH-25 | Feedback Loop 3: Decision capture integration — Add decision capture prompts to `/processes:plan` (Phase 2: alternatives considered) and `/processes:work` (non-trivial implementation choices). Link decisions to affected files in code index. | 2026-02-28 | plan:feedback-loops |
| ARCH-26 | Feedback Loop 3: Decision index generator — Script `regenerate-decision-index.js` that scans `.claude/decisions/` and generates searchable INDEX.md (Date, Decision, Tags, Files table). Also adds `decisions` field to code index file entries for cross-reference. | 2026-02-28 | plan:feedback-loops |
| INFRA-68 | Index V2: Add test coverage map — Cross-reference service/route files with test files (`X.test.ts` or `__tests__/X.test.ts`). Add `tests` field to index entries: `{ file, exists, testCount }`. ~10% token increase, signals coverage gaps. | 2026-02-28 | plan:feedback-loops |
| PERF-7 | Lazy-loading heavy frontend components | 2026-02-28 | roadmap |
...
| INFRA-65 | Feedback Loop 4: Production signal reader for `/processes:begin` — Script `read-production-signals.js` that reads `.claude/production/signals.json`, formats unresolved signals with severity/frequency for session start display. Add section to `begin.md` after git status. | 2026-02-28 | plan:feedback-loops |
| INFRA-66 | Feedback Loop 4: Manual signal ingestion CLI — Script `add-production-signal.js` accepting `--type`, `--severity`, `--message`, `--file`, `--line` flags. Also `--resolve sig-001` to mark resolved. For teams without Sentry integration. | 2026-02-28 | plan:feedback-loops |
| INFRA-67 | Index V2: Deduplicate legend — Move decode legend (repeated 8x across CODEBASE-*.md, ~288 lines, ~1,500 tokens wasted per multi-domain load) to single `.claude/code-index-legend.md`. Update `regenerate-code-index.js` to not emit legend per file. Update `load-code-index.js` to load legend once. | 2026-02-28 | plan:feedback-loops |
